...
public class SubscriptionsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<Subscriptioned> SubscriptionsList;
    ...
    class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        ImageView overflow,share,poster,channelprofile;
    ...
    public SubscriptionsAdapter(Context mContext, List<Subscriptioned> subscriptionsList) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            case POST_ITEM_VIEW_TYPE:
                View itemView = inflater.inflate(R.layout.subscriptions_card, parent, false);
               holder = new MyViewHolder(itemView);
                ...
                return holder;
            default:
                 itemView = inflater.inflate(R.layout.subscriptions_card, parent, false);
                 holder = new MyViewHolder(itemView);
                return holder;
        }
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder2, final int position) {
        ...
        switch (viewType) {
            case UNIFIED_NATIVE_AD_VIEW_TYPE:
                List SubscriptionsList2 = this.SubscriptionsList;
                UnifiedNativeAd nativeAd = (UnifiedNativeAd) SubscriptionsList2.get(position);
                populateNativeAdView(nativeAd, ((UnifiedNativeAdViewHolder) holder2).getAdView());
                break;
            case POST_ITEM_VIEW_TYPE:
                final MyViewHolder holder = (MyViewHolder) holder2;
                holder.channelname.setText( SubscriptionsList.get(position).getChannelName());
                holder.channelid.setText(SubscriptionsList.get(position).getChannelId());
                holder.videodesc.setText(SubscriptionsList.get(position).getVideoDesc());
                holder.thischannelownerid.setText(SubscriptionsList.get(position).getChannelownerid());
                holder.thisvideoid.setText(SubscriptionsList.get(position).getVideoId());
                holder.thisvideourl.setText(SubscriptionsList.get(position).getVideourl());
                holder.channelowner.setText(SubscriptionsList.get(position).getPosterName());
                holder.totalvideos.setText(SubscriptionsList.get(position).getTotalVideos());
                holder.videolikes.setText(SubscriptionsList.get(position).getNumOfBlesses());
                holder.videoviews.setText(SubscriptionsList.get(position).getNumOfComments());
                holder.duration.setText(SubscriptionsList.get(position).getVDuration());
                holder.videosize.setText(SubscriptionsList.get(position).getVideosize());
                holder.channelname.setTag(SubscriptionsList.get(position).getChannelCover());
                holder.channelid.setTag(SubscriptionsList.get(position).getTotalVideos());
                String issubscribed = SubscriptionsList.get(position).getSubscribed();

                // loading video poster using Glide library
                Glide.with(holder.itemView.getContext()).load(SubscriptionsList.get(position).getPoster()).into(holder.poster);
                Glide.with(holder.itemView.getContext()).load(SubscriptionsList.get(position).getChannelProfile()).placeholder(R.drawable.gallerysmall).circleCrop().into(holder.channelprofile);
        }
    }
    @Override
    ...
    public long getItemId(int position) {
        Object recyclerViewItem = SubscriptionsList.get(position);
        if (recyclerViewItem instanceof UnifiedNativeAd) {
            return position;
        }
        return SubscriptionsList.get(position).getId();
    }
    @Override
