...
public class LikedAdapter extends RecyclerView.Adapter<LikedAdapter.MyViewHolder> {
    ...
    public interface OnRecyclerViewItemClickListener {
        void onItemLikedClicked(String[] videoinfos);}
    ...
    public LikedAdapter(Context mContext, List LikedList) {
    ...
    public LikedAdapter.MyViewHolder onCreateViewHolder(@NonNull  ViewGroup parent, int viewType) {
        View itemView = inflater.inflate(R.layout.liked_card, parent, false);
        ...
        itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mListener.onItemLikedClicked(videoinfo);
    ...
    @Override
    public void onBindViewHolder(@NonNull final LikedAdapter.MyViewHolder holder, final int position) {
        holder.channelname.setText(LikedList.get(position).getChannelName());
        holder.channelid.setText(LikedList.get(position).getChannelId());
        holder.videodesc.setText(LikedList.get(position).getVideoDesc());
        holder.postdate.setText(LikedList.get(position).getTotalVideos());
        holder.thischannelownerid.setText(LikedList.get(position).getChannelownerid());
        holder.thisvideoid.setText(LikedList.get(position).getVideoId());
        Toast.makeText(mContext, "id "+LikedList.get(position).getVideoId(), Toast.LENGTH_SHORT).show();
        holder.thisvideourl.setText(LikedList.get(position).getVideourl());
        holder.channelowner.setText(LikedList.get(position).getPosterName());
        holder.totalvideos.setText(LikedList.get(position).getTotalVideos());
        holder.videolikes.setText(LikedList.get(position).getNumOfBlesses());

        // loading video poster using Glide library
        Glide.with(mContext).load(LikedList.get(position).getPoster()).transform(new FitCenter(), new RoundedCorners(10)).into(holder.poster);
        Glide.with(mContext).load(LikedList.get(position).getChannelProfile()).apply(RequestOptions.circleCropTransform()).into(holder.channelprofile);
