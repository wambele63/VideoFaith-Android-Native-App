...
public class Favouritesfragment extends Fragment implements
    ...
    private RecyclerView recyclerViewfavourited;
    private Favouritesfragment.OnFragmentInteractionListener mListener;
    private Box<Favourites> favouritedbox = ObjectBox.getBoxStore().boxFor(Favourites.class);
    private List favouritedList;
    private FavouritesAdapter radapter;
    ...
    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle savedInstanceState) {
        inflate = layoutInflater.inflate(R.layout.favourites_fragment, viewGroup, false);
        fetchfavouritedJSON();
        return inflate;
    }
    ...
    @SuppressLint("StaticFieldLeak")
    private void fetchfavouritedJSON() {
        try {
            db.collection("favourites").whereEqualTo("favouriter", Constants.myId)
                    .orderBy("timestamp", Query.Direction.DESCENDING)
                    .get().addOnCompleteListener(getActivity(),task -> {
                favouritedbox.removeAll();
                if (task.isSuccessful() && !task.getResult().isEmpty()) {
                    for (QueryDocumentSnapshot document : task.getResult()) {
                        db.collection("videos").document(document.get("favourited") + "")
                                .get().addOnCompleteListener(getActivity(), task1 -> {
                                    if (task1.isSuccessful()) {
                                        final Map<String, Object> post = task1.getResult().getData();
                                        final Favourites videopost = new Favourites();
                                        ...
                                        final String favourites = post.get("favourites") + "";
                                        ...
                                        db.collection("channels").document(channelid).get().addOnCompleteListener(getActivity(),new OnCompleteListener<DocumentSnapshot>() {
                                            ...
                                            public void onComplete(@NonNull Task<DocumentSnapshot> task1) {
                                                if (task1.isSuccessful()) {
                                                    ...
                                                    videopost.setNumOfBlesses(favourites + " Amens");
                                                    ...
                                                    videopost.setNumOfAmens(favourites + " Amens");
                                                    videopost.setNumOfComments(views + " Views");
                                                    favouritedbox.put(videopost);
                                                }
                                                attachfavourited();
                                            }
                                        });
    ...
    public void onViewCreated(@Nullable final View view, @Nullable Bundle savedInstanceState) {
        ...
        recyclerViewfavourited = view.findViewById(R.id.recycler_view_favourites);
        attachfavourited();
    }
    @Override
    ...
    private void attachfavourited() {
        recyclerViewfavourited.setAdapter(null);
        favouritedList = favouritedbox.getAll();
        radapter = new FavouritesAdapter(this.getContext(), favouritedList);
        radapter.setOnRecyclerViewItemClickListener(new FavouritesAdapter.OnRecyclerViewItemClickListener()
        {
            @Override
            public void onItemFavouritesClicked(String[] videoinfo) {
        ...
        recyclerViewfavourited.setHasFixedSize(true);
        recyclerViewfavourited.setItemViewCacheSize(20);
        recyclerViewfavourited.setDrawingCacheEnabled(true);
        recyclerViewfavourited.setDrawingCacheQuality(View.DRAWING_CACHE_QUALITY_HIGH);
        SlideInBottomAnimationAdapter alphaAdapter = new SlideInBottomAnimationAdapter(radapter);
        recyclerViewfavourited.setAdapter(new ScaleInAnimationAdapter(alphaAdapter));
        recyclerViewfavourited.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
        ...
        recyclerViewfavourited.setLayoutManager(mLayoutManager);
    }
    ...
    public void onAttach(Context context) {
        ...
        if (context instanceof Favouritesfragment.OnFragmentInteractionListener) {
            this.mListener = (Favouritesfragment.OnFragmentInteractionListener) context;
            return;
        }
    ...
    private void insertAdsInMenuItems() {
        ...
        int offset = (favouritedList.size() / mNativeAds.size())+1;
        int index = 0;
        for (UnifiedNativeAd ad: mNativeAds) {
            favouritedList.add(index, ad);
            index = index + offset;
        }
    }
